...
def generate_vocab_txt(dataset):
    vocab = []
    for sentence in dataset:
        for word in sentence.split():
            if (word.isalnum()):
                vocab.append(word.lower())
    file_object = open('data/polaritydata.vocab', 'w')
    for word in vocab:
...
def train(x_train, y_train, vocab_processor, x_dev, y_dev, embedding):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            sess.run(tf.global_variables_initializer(), feed_dict={cnn.embedding_placeholder : embedding})
...
def main(argv=None):
    x_test, y = read_source_data()
    generate_vocab_txt(x_test)
